...
class Animation
{
	 ...
	 */
	public function new(Parent:AnimationController, Name:String, Frames:Array<Int>, FrameRate:Int = 0, Looped:Bool = true, FlipX:Bool = false, FlipY:Bool = false)
	{
		this.parent = Parent;
		this.name = Name;
		frameRate = FrameRate;
	...
	public function play(Force:Bool = false, Reversed:Bool = false, Frame:Int = 0):Void
	{
		...
		if (Frame < 0)
		{
			curFrame = glaze.util.Random.RandomInteger(0,numFramesMinusOne);
		}
		else
		{
			curFrame = Frame;
	...
	public function clone(Parent:AnimationController):Animation
	{
		return new Animation(Parent, name, _frames, frameRate, looped, flipX, flipY);
	}
